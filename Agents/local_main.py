...
async def run_with_session_rollover(initial_prompt: str, max_rounds: int = 8) -> dict:
    ...
    while True:
        ...
        async with InMemoryRunner(agent=agent_instance, app_name="AdGenLocal") as runner:
        ...
        if status.lower() == "continue" and rounds < max_rounds:
            # Create a fresh session by reloading the module and re-grabbing root_agent
            try:
                reload(master_agent_module)
                agent_instance = master_agent_module.root_agent
            except Exception as e:
                logger.warning("Failed to reload MasterAgent module; reusing existing agent. err=%s", e)
                agent_instance = root_agent
