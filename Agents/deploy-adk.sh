...
if [ "$SERVICE_URL" != "Unable to get URL" ]; then
    ...
    echo -e "${GREEN}🔔 Pub/Sub Endpoint: ${SERVICE_URL}/pubsub/push${NC}"
fi
...
echo ""
echo -e "${BLUE}🔔 Pub/Sub Integration:${NC}"
echo "   ✅ Pub/Sub push endpoint available at /pubsub/push"
echo "   ✅ Health check endpoint available at /health"
echo "   ✅ Hybrid mode: ADK UI + Pub/Sub triggers"

echo ""
echo -e "${YELLOW}📝 Next steps:${NC}"
echo "1. Test your agent at the service URL"
echo "2. Use the UI to interact with your agents"
echo "3. Set up Pub/Sub topic and subscription:"
echo "   ./setup-pubsub.sh"
echo "4. Check logs with: gcloud logs tail --service=${SERVICE_NAME} --region=${REGION}"
echo ""
...
echo "   Setup Pub/Sub: ./setup-pubsub.sh"
